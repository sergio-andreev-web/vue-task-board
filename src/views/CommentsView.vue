...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('comments', schema);
const draft = reactive({ name: '', author: '', reactions: 0, pinned: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', author: '', reactions: 0, pinned: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Comments</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Author <input type="text" v-model="draft.author"  /></label>
      <label>Reactions <input type="number" min="0" v-model.number="draft.reactions"  /></label>
      <label>Pinned <input type="checkbox" v-model="draft.pinned"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search comments" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'comments')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Author</th><th>Reactions</th><th>Pinned</th><th>Actions</th></tr></thead>
      <tbody>
