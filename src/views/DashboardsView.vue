...
const schema = [
  ...
  { key: 'widgets', label: 'Widgets', type: 'number' },
  { key: 'public', label: 'Public', type: 'boolean' },
];
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('dashboards', schema);
const draft = reactive({ name: '', layout: '', widgets: 0, public: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', layout: '', widgets: 0, public: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Dashboards</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Layout <input type="text" v-model="draft.layout"  /></label>
      <label>Widgets <input type="number" min="0" v-model.number="draft.widgets"  /></label>
      <label>Public <input type="checkbox" v-model="draft.public"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search dashboards" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'dashboards')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Layout</th><th>Widgets</th><th>Public</th><th>Actions</th></tr></thead>
      <tbody>
        <tr v-for="record in visible" :key="record.id">
        <td>{{ record.name }}</td>
        <td>{{ record.layout }}</td>
        <td>{{ record.widgets }}</td>
        <td>{{ record.public }}</td>
