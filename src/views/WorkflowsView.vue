...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('workflows', schema);
const draft = reactive({ name: '', trigger: '', runs: 0, enabled: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', trigger: '', runs: 0, enabled: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Workflows</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Trigger <input type="text" v-model="draft.trigger"  /></label>
      <label>Runs <input type="number" min="0" v-model.number="draft.runs"  /></label>
      <label>Enabled <input type="checkbox" v-model="draft.enabled"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search workflows" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'workflows')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Trigger</th><th>Runs</th><th>Enabled</th><th>Actions</th></tr></thead>
      <tbody>
