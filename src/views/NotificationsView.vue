...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('notifications', schema);
const draft = reactive({ name: '', channel: '', attempts: 0, read: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', channel: '', attempts: 0, read: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Notifications</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Channel <input type="text" v-model="draft.channel"  /></label>
      <label>Attempts <input type="number" min="0" v-model.number="draft.attempts"  /></label>
      <label>Read <input type="checkbox" v-model="draft.read"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search notifications" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'notifications')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Channel</th><th>Attempts</th><th>Read</th><th>Actions</th></tr></thead>
      <tbody>
