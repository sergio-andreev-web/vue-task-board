...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('activities', schema);
const draft = reactive({ name: '', actor: '', sequence: 0, visible: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', actor: '', sequence: 0, visible: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Activities</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Actor <input type="text" v-model="draft.actor"  /></label>
    ...
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search activities" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'activities')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Actor</th><th>Sequence</th><th>Visible</th><th>Actions</th></tr></thead>
      <tbody>
