...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('labels', schema);
const draft = reactive({ name: '', color: '', position: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', color: '', position: 0, active: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Labels</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Color <input type="text" v-model="draft.color"  /></label>
    ...
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search labels" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'labels')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Color</th><th>Position</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
