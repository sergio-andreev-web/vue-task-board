...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('webhooks', schema);
const draft = reactive({ name: '', url: '', retries: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', url: '', retries: 0, active: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Webhooks</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Retries <input type="number" min="0" v-model.number="draft.retries"  /></label>
      <label>Active <input type="checkbox" v-model="draft.active"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search webhooks" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'webhooks')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Url</th><th>Retries</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
        <tr v-for="record in visible" :key="record.id">
        <td>{{ record.name }}</td>
        <td>{{ record.url }}</td>
        <td>{{ record.retries }}</td>
        <td>{{ record.active }}</td>
