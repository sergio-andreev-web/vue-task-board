...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('invitations', schema);
const draft = reactive({ name: '', email: '', attempts: 0, accepted: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', email: '', attempts: 0, accepted: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Invitations</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Email <input type="text" v-model="draft.email"  /></label>
      <label>Attempts <input type="number" min="0" v-model.number="draft.attempts"  /></label>
      <label>Accepted <input type="checkbox" v-model="draft.accepted"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search invitations" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'invitations')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Email</th><th>Attempts</th><th>Accepted</th><th>Actions</th></tr></thead>
      <tbody>
