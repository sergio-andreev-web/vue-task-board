...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('releases', schema);
const draft = reactive({ name: '', version: '', build: 0, published: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', version: '', build: 0, published: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Releases</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Version <input type="text" v-model="draft.version"  /></label>
      <label>Build <input type="number" min="0" v-model.number="draft.build"  /></label>
      <label>Published <input type="checkbox" v-model="draft.published"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search releases" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'releases')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Version</th><th>Build</th><th>Published</th><th>Actions</th></tr></thead>
      <tbody>
