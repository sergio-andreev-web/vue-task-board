...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('expenses', schema);
const draft = reactive({ name: '', category: '', amount: 0, approved: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', category: '', amount: 0, approved: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Expenses</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Category <input type="text" v-model="draft.category"  /></label>
      <label>Amount <input type="number" min="0" v-model.number="draft.amount"  /></label>
      <label>Approved <input type="checkbox" v-model="draft.approved"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search expenses" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'expenses')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Category</th><th>Amount</th><th>Approved</th><th>Actions</th></tr></thead>
      <tbody>
