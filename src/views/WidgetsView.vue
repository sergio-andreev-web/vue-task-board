...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('widgets', schema);
const draft = reactive({ name: '', kind: '', position: 0, visible: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', kind: '', position: 0, visible: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Widgets</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Kind <input type="text" v-model="draft.kind"  /></label>
    ...
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search widgets" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'widgets')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Kind</th><th>Position</th><th>Visible</th><th>Actions</th></tr></thead>
      <tbody>
