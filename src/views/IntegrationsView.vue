...
const schema = [
  ...
  { key: 'syncs', label: 'Syncs', type: 'number' },
  { key: 'active', label: 'Active', type: 'boolean' },
];
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('integrations', schema);
const draft = reactive({ name: '', provider: '', syncs: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', provider: '', syncs: 0, active: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Integrations</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Provider <input type="text" v-model="draft.provider"  /></label>
      <label>Syncs <input type="number" min="0" v-model.number="draft.syncs"  /></label>
      <label>Active <input type="checkbox" v-model="draft.active"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search integrations" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'integrations')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Provider</th><th>Syncs</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
