...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('sprints', schema);
const draft = reactive({ name: '', project: '', velocity: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', project: '', velocity: 0, active: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Sprints</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Velocity <input type="number" min="0" v-model.number="draft.velocity"  /></label>
      <label>Active <input type="checkbox" v-model="draft.active"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search sprints" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'sprints')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Project</th><th>Velocity</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
