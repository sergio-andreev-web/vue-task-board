...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('reminders', schema);
const draft = reactive({ name: '', user: '', delay: 0, sent: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', user: '', delay: 0, sent: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Reminders</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Delay <input type="number" min="0" v-model.number="draft.delay"  /></label>
      <label>Sent <input type="checkbox" v-model="draft.sent"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search reminders" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'reminders')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>User</th><th>Delay</th><th>Sent</th><th>Actions</th></tr></thead>
      <tbody>
