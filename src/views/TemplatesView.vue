...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('templates', schema);
const draft = reactive({ name: '', description: '', uses: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', description: '', uses: 0, active: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Templates</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Description <input type="text" v-model="draft.description"  /></label>
    ...
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search templates" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'templates')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Description</th><th>Uses</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
