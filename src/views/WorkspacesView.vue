...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('workspaces', schema);
const draft = reactive({ name: '', owner: '', seats: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', owner: '', seats: 0, active: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Workspaces</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Owner <input type="text" v-model="draft.owner"  /></label>
      <label>Seats <input type="number" min="0" v-model.number="draft.seats"  /></label>
      <label>Active <input type="checkbox" v-model="draft.active"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search workspaces" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'workspaces')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Owner</th><th>Seats</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
