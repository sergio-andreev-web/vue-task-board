...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('milestones', schema);
const draft = reactive({ name: '', project: '', progress: 0, done: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', project: '', progress: 0, done: false });
...
<template>
  <section class="collection-view">
    <header class="collection-header">
      <div><p class="eyebrow">Workspace</p><h2>Milestones</h2></div>
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Project <input type="text" v-model="draft.project"  /></label>
      <label>Progress <input type="number" min="0" v-model.number="draft.progress"  /></label>
      <label>Done <input type="checkbox" v-model="draft.done"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search milestones" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'milestones')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Project</th><th>Progress</th><th>Done</th><th>Actions</th></tr></thead>
      <tbody>
