...
const { records, create, update, remove, archive, restore, clearArchived, duplicate, importJson } = useCollection('projects', schema);
const draft = reactive({ name: '', owner: '', budget: 0, active: false });
const query = ref('');
...
function addRecord() {
  try {
    create(draft);
    error.value = '';
    Object.assign(draft, { name: '', owner: '', budget: 0, active: false });
...
<template>
  <section class="collection-view">
    ...
    <form class="collection-form" @submit.prevent="addRecord">
      ...
      <label>Budget <input type="number" min="0" v-model.number="draft.budget"  /></label>
      <label>Active <input type="checkbox" v-model="draft.active"  /></label>
      <button type="submit">Create</button>
    </form>
    <p v-if="error" class="error" role="alert">{{ error }}</p>
    <div class="collection-toolbar">
      <input v-model="query" placeholder="Search projects" aria-label="Search records" />
      ...
      <button @click="exportRecords(records, 'projects')">Export</button>
      <label class="import">Import <input type="file" accept="application/json" @change="loadFile" /></label>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Name</th><th>Owner</th><th>Budget</th><th>Active</th><th>Actions</th></tr></thead>
      <tbody>
