...
const message = ref('');
const currentView = ref('taskBoard');
const selectedView = computed(() => views[currentView.value]?.component);
...
<template>
  <main>
    <nav class="view-nav" aria-label="Workspace sections">
      <button :class="{ selected: currentView === 'taskBoard' }" @click="currentView = 'taskBoard'">Task Board</button>
      <button v-for="(view, key) in views" :key="key" :class="{ selected: currentView === key }" @click="currentView = key">{{ view.label }}</button>
    </nav>
